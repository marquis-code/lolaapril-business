<template>
  <div class="min-h-screen">
    <div 
      ...
      :class="{ 'p-0': isMaximized, 'p-4 sm:p-6 lg:p-8': !isMaximized }"
    >
      <!-- Filters (only show when not maximized) -->
      <section v-if="!isMaximized" class="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h2 class="text-xl font-bold text-gray-900">Appointments Calendar</h2>
            <p class="text-xs text-gray-500">View and manage appointments</p>
          </div>
          <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="w-[160px] relative">
              <label class="absolute -top-2 left-2 bg-white px-1 text-[10px] font-medium text-gray-400 z-10">Go to Date</label>
              <input
                v-model="goToDateInput"
                type="date"
                @change="navigateToDate"
                class="w-full h-11 px-3 rounded-xl border border-gray-100 text-sm focus:border-[#005967] focus:ring-[#005967] focus:ring-1 outline-none transition-all"
              />
            </div>
            <button
              @click="handleTodayClick"
              class="h-11 px-6 rounded-xl text-sm font-bold text-[#005967] border border-gray-100 hover:bg-gray-50 transition-all active:scale-95"
            >
              Today
            </button>
          </div>
      ...
      <!-- Calendar Section -->
      <section 
        class="flex-grow border border-gray-100 rounded-2xl overflow-hidden shadow-2xl relative"
        :class="{ 'rounded-none border-0': isMaximized }"
      >
        <UiCustomCalendar 
          v-model="currentDate"
          :appointments="filteredAppointments" 
          :is-maximized="isMaximized"
          @select-appointment="openAppointmentDetails"
...
const navigateToDate = () => {
  if (goToDateInput.value) {
    currentDate.value = new Date(goToDateInput.value)
  }
}
